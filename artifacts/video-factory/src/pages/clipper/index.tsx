...
import {
  ...
  TrendingUp, MessageSquare, Share2, Eye,
} from "lucide-react";
...
export default function ClipperPage() {
  ...
  const analyzeTranscript = async () => {
    ...
    try {
      ...
      setClips(data.clips ?? []);
      if (!data.clips?.length) setError("No viral clips found. Try a different video.");
    } catch {
          ...
          {transcript && (
            <>
              ...
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
                ...
                <div ref={txRef} className="max-h-64 overflow-y-auto p-5 space-y-1">
                  {segments.map((seg, i) => (
              ...
              <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-sm text-gray-900">Step 2 — Analyze for Viral Clips</span>
                    <span className="text-xs text-gray-400">AI-powered (Claude Haiku)</span>
                  </div>
                  <Button
                    onClick={analyzeTranscript}
                    disabled={analyzing}
                    className="bg-amber-400 hover:bg-amber-500 text-amber-950 font-semibold text-sm h-8 px-5"
                  >
                    {analyzing
                ...
                {analyzing && (
                  <p className="text-xs text-gray-400 mt-3">
                    Analyzing {segments.length} transcript segments for viral opportunities…
                  </p>
                )}
              </div>
            </>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-sm text-gray-900">
                  {clips.length} Viral Clips Found
                </h2>
                <span className="text-xs text-gray-400">Sorted by viral potential</span>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100 bg-gray-50">
                      ...
                      <th className="text-left px-4 py-2.5 text-xs font-medium text-gray-500">Timestamps</th>
                      <th className="text-left px-4 py-2.5 text-xs font-medium text-gray-500">Topic</th>
                      <th className="text-left px-4 py-2.5 text-xs font-medium text-gray-500">Hook</th>
                      ...
                      .map(clip => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            ...
                            <span className="text-xs text-gray-400 ml-1.5">({clip.duration})</span>
                          </td>
                          <td className="px-4 py-3 text-xs text-gray-700 max-w-[200px] truncate">{clip.topic}</td>
                          <td className="px-4 py-3">
                            <span className={cn(
                          ...
                          <td className="px-4 py-3 text-xs text-gray-500">{clip.platform}</td>
                          <td className="px-4 py-3">
                  ...
                  <div className="bg-white rounded-xl border border-violet-100 shadow-sm p-6 space-y-5">
                    ...
                    <div className="flex items-center gap-6 p-4 bg-gray-50 rounded-lg">
                      ...
                      <div className="ml-auto text-right">
                        <div className={cn(
                          "text-2xl font-black",
                          clip.viralScore >= 9 ? "text-emerald-500" : "text-amber-500"
                        )}>
                          {clip.viralScore}/10
                        </div>
                    ...
                    {/* Hook */}
                    <div>
                      <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">Hook</p>
                      <p className="text-base font-bold text-gray-900 bg-amber-50 border border-amber-100 rounded-lg px-4 py-2.5">
                        "{clip.hook}"
                      </p>
                    </div>
                        ...
                        {clip.titleIdeas?.slice(0, 5).map((t, i) => (
                          <div key={i} className="flex items-center justify-between gap-2 px-3 py-1.5 rounded-lg bg-gray-50 border border-gray-100">
                            ...
                            <Button
                              ...
                              onClick={() => copyText(t, `title-${clip.id}-${i}`)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setClipDialog(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-violet-50 rounded-lg p-3 text-sm">
              ...
              <p className="text-violet-600 text-xs mt-0.5 font-mono">
                {clipDialog.startTime} → {clipDialog.endTime} ({clipDialog.duration})
              </p>
            </div>
            ...
            <div>
              ...
              <Input
                value={clipVideoUrl}
                onChange={e => setClipVidUrl(e.target.value)}
                placeholder="https://example.com/video.mp4"
                className="text-sm h-9"
              />
              <p className="text-xs text-gray-400 mt-1">
                Paste a direct video download link. The clip will be face-detected and centered automatically.
              </p>
            </div>
            ...
            {clipToken && (
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-emerald-600">Face detected and centered automatically.</p>
                </div>
                <a
                  href={`/api/clipper/download/${clipToken}`}
                  download="clip.mp4"
            ...
            <div className="flex gap-3 pt-1">
              <Button variant="outline" onClick={() => setClipDialog(null)} className="flex-1 h-9 text-sm">
                Cancel
              </Button>
              <Button
                onClick={extractClip}
                disabled={clipping || !clipVideoUrl.trim()}
